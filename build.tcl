...
proc configure {args} {
    ...
    set d [dict create snappy $snappy static $static ldir $ldir idir $idir config $config]
    return $d
}
proc Hinstall {} { return "?destination? ?config? ?-snappy <path>? ?-static? ?-dynamic?\n\tInstall all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-snappy <path> = path to Crossroads I/O\n\t-static = link Crossroads I/O statically, default is to link dynamically\n\t-dynamic = link Crossroads I/O dynamically" }
proc _install {args} {
    global packages
	...
	if {[string length $snappy]} {
	    lappend rcargs -L [file join $snappy lib] -I [file join $snappy include]
	} else {
...
proc Hdebug {} { return "?destination? ?config? ?-snappy <path>? ?-static? ?-dynamic?\n\tInstall debug builds of all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-snappy <path> = path to Crossroads I/O\n\t-static = link Crossroads I/O statically, default is to link dynamically\n\t-dynamic = link Crossroads I/O dynamically" }
proc _debug {args} {
    global packages
    set d [eval configure $args]
    dict with d {}
...
proc _gui {} {
    global INSTALLPATH SNAPPYPATH BUILDSTATIC
    package require Tk
#    package require widget::scrolledwindow
    ...
    label  .lz -text {Crossroads I/O Path: }
    entry  .ez -textvariable ::SNAPPYPATH
    button .bz -command [list Browse ::SNAPPYPATH] -text Browse
    checkbutton .bs -text "Link Crossroads I/O statically" -variable BUILDSTATIC -anchor w
...
proc Install {} {
    global INSTALLPATH SNAPPYPATH NOTE
	...
	_install $INSTALLPATH -snappy $SNAPPYPATH %s
...
proc Debug {} {
    global INSTALLPATH SNAPPYPATH
    foreach p {.ei .ez .bi .bz .bs .i .d} {
	$p configure -state disabled
    }

    set fail [catch [format {
	_debug $INSTALLPATH -snappy $SNAPPYPATH %s
